...
def saveProjectPlans(request, slug):
    projectPlans = ProjectPlan.objects.filter(create_user_id=request.user, project_id=slug);
    projectPlans.delete()
...
def loadProjectPlans(request, slug):
    plans = ProjectPlan.objects.filter(project_id=slug, create_user_id=request.user.id)
    data = {}
    if plans:
